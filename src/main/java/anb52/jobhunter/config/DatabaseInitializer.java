...
// Fake data với CommandLineRunner
@Service
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        long countPermissions = this.permissionRepository.count(); // đếm số lượng bản ghi hiện có trong permission
        long countRoles = this.roleRepository.count(); // đếm số lượng bản ghi hiện có trong role
        long countUsers = this.userRepository.count(); // đếm số lượng bản ghi hiện có trong user

        // Nếu không có permission nào, thì tạo danh sách Permission
        ...
        // Nếu không có role, lấy tất cả permission và tạo role SUPER_ADMIN
        if (countRoles == 0) {
        ...
        // Nếu không có user, tạo user mới (adminUser) gắn vai trò SUPER_ADMIN
        if (countUsers == 0) {
